...
# env = gym.make("tarware-medium-14agvs-7pickers-partialobs-v1")
...
for episode in range(args.num_episodes):
    state = env.reset()
    graph_observation_space_mapper = MultiAgentGraphObservationSpace(19, 9, (35,22), 240, normalised_coordinates=True)

    hetero_data = graph_observation_space_mapper._build_graph_from_extracted_info(env)
    ...
    while not done and step < 1000:  # Max steps per episode
        ...
        next_hetero_data = graph_observation_space_mapper._build_graph_from_extracted_info(env)
